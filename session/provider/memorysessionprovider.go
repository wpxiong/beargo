...
func porcessSessionSerialize(sessInfo *session.SessionInfo,sess *session.Session) {
  if len(sess.SessionSerializeInfo) == 0 {
     sessInfo.IsSerialized = false
  }
}
...
func (provider *MemorySessionProvider) DeseriazeObject (valueId string,bytearray []byte, obj interface{},sess *session.Session) bool  {
  sessInfo := provider.SessionInfoMap[sess.SessionId]
  defer porcessSessionSerialize(sessInfo,sess)
  switch reflect.TypeOf(obj).Kind() {
     case reflect.Ptr:
        if bytearray,ok := sess.SessionSerializeInfo[valueId]; ok {
           delete(sess.SessionSerializeInfo,valueId)
           return session.DeseriazeObject(bytearray,obj)
        }else {
           return false
        }
     default:
